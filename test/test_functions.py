...
from roppy.functions import curl, divergence, kinetic_energy
...
class test_kinetic_energy(unittest.TestCase):
    ...
    def rest_isotropy(self):
        ...
        theta = 0.1
        U = np.array([[vel(-0.5, 0.0, theta)[0], vel(0.5, 0.0, theta)[0]]])
        V = np.array([[vel(0.0, -0.5, theta)[1], vel(0.0, 0.5, theta)[1]]]).T
        kinetic_energy(U, V)
