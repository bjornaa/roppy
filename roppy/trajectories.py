...
def movepart(grid, U, V, X0, Y0, dt, nstep, order=4):
    ...
    if order == 1:
        for t in range(nstep - 1):
            pm = roppy.sample2D(grid.pm, X[t, :] - i0, Y[t, :] - j0, outside_value=0)
            pn = roppy.sample2D(grid.pn, X[t, :] - i0, Y[t, :] - j0, outside_value=0)
            Ut = roppy.sample2D(U, X[t, :] - i0 + 0.5, Y[t, :] - j0, outside_value=0)
            Vt = roppy.sample2D(V, X[t, :] - i0, Y[t, :] - j0 + 0.5, outside_value=0)
            X[t + 1, :] = X[t, :] + Ut * dt * pm
            Y[t + 1, :] = Y[t, :] + Vt * dt * pn
    ...
    # Runge-Kutta 4th order
        ...
        for t in range(nstep - 1):
            ...
            x0 = X[t, :] - i0
            y0 = Y[t, :] - j0
            dx1 = pm * dt * roppy.sample2D(U, x0 + 0.5, y0, outside_value=0)
            dy1 = pm * dt * roppy.sample2D(V, x0, y0 + 0.5, outside_value=0)
            dx2 = (
                pm
                * dt
                * roppy.sample2D(
                    U, x0 + 0.5 * dx1 + 0.5, y0 + 0.5 * dy1, outside_value=0
                )
            )
            dy2 = (
                pm
                * dt
                * roppy.sample2D(
                    V, x0 + 0.5 * dx1, y0 + 0.5 * dy1 + 0.5, outside_value=0
                )
            )
            dx3 = (
                pm
                * dt
                * roppy.sample2D(
                    U, x0 + 0.5 * dx2 + 0.5, y0 + 0.5 * dy2, outside_value=0
                )
            )
            dy3 = (
                pm
                * dt
                * roppy.sample2D(
                    V, x0 + 0.5 * dx2, y0 + 0.5 * dy2 + 0.5, outside_value=0
                )
            )
            dx4 = pm * dt * roppy.sample2D(U, x0 + dx3 + 0.5, y0 + dy3, outside_value=0)
            dy4 = pm * dt * roppy.sample2D(V, x0 + dx3, y0 + dy3 + 0.5, outside_value=0)
...
def curly_vectors(grid, U, V, stride=5, dt=3600, nstep=72, order=4):
    ...
    return movepart(grid, U, V, X, Y, dt=dt, nstep=nstep, order=order)
