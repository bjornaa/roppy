...
sec_dict = {
    'Feie-Shetland' : {'lat0': 60.750, 'lon0':  4.617,
                       'lat1': 60.750, 'lon1': -0.667},
    'Utsira-W'      : {'lat0': 59.283, 'lon0':  5.033,
                       'lat1': 59.283, 'lon1': -2.223},
    'Torungen-Hirtshals' :
                      {'lat0': 58.400, 'lon0':  8.767,
                       'lat1': 57.633, 'lon1':  9.867},
    }
...
for t in range(ntimes):
    ...
    outstring = "".join(["{:10.3f}{:10.3f}".format(nf, pf)
                         for (nf, pf) in zip(netflux, posflux)])
    f1.write(outstring + "\n")
